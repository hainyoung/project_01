...
def speed_digit_recognizer(image,x,y,h,w):
        ...
        for cnt_speed in contours_speed:
            if cv2.contourArea(cnt_speed)<700:           
...
def date_time_recognizer(image,x,y,h,w):
        samples_dt = np.loadtxt('./problem_solved/date_time_generalsamples.data', np.float32)
        responses_dt = np.loadtxt('./problem_solved/date_time_generalresponses.data', np.float32)
